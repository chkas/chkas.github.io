...
+ We set the drawing pen with *move_pen* to position (10 20), that is 10 from the left and 20 from above. From there, we draw with *draw_line* a line to position (50 40).

+de Wir setzen mit *move_pen* den Zeichenstift auf die Position (10 20), das ist 10 von links und 20 von oben. Von dort ziehen wir dann mit *draw_line* eine Linie auf die Position (50 40).
...
+ With the command *draw_rect* you can draw filled rectangles. The parameters specify the width and height.

+de Mit dem Befehl *draw_rect* kann man gefüllte Rechtecke zeichnen. Die Parameter geben die Breite und die Höhe an.

+ *set_color* sets the drawing set_color. *set_color 900*, for example, is a rich red set_color.

+de *set_color* setzt die Zeichenfarbe. *set_color 900* ist zum Beispiel eine satte rote Farbe.
...
# roof
set_color 722
draw_polygon [ 45 30 10 55 80 55 ]
# windows
set_color 555
move_pen 25 60
draw_rect 10 10
...
+ *draw_text* writes a text to the drawing area. *draw_polygon [x1 y1 x2 y2 ..]* draws a filled polygon with the given coordinates.

+de *draw_text* schreibt einen Text auf die Zeichenfläche. *draw_polygon [x1 y1 x2 y2 ..]* zeichnet ein gefülltes Polygon mit den angegebenen Koordinaten.
...
+ The command *draw_circle* draws a filled circle on the current position.

+ *set_linewidth* sets the line width. The lines are rounded at the ends.

+de Mit dem Befehl *draw_circle* wird ein gefüllter Kreis an der aktuellen Position gezeichnet. 

+de *set_linewidth* stellt die Linienstärke ein. Die Linien sind an den Enden abgerundet.
...
if a > 20
  ...
  print "This number is not large."
end
print a
...
+ *Task 2:* Connect the stars of the constellation *Cassiopeia*, which can easily be recognized by its W-shape. To find the indices of the stars, output them using *draw_text i* when drawing the star map.

+de *Aufgabe 2:* Verbinde die Sterne des Sternbildes *Cassiopeia*, welches man leicht an seiner W-Form erkennen kann. Um die Indizes der Sterne zu ermitteln, gibst du diese beim Zeichnen der Sternkarte einfach mittels *draw_text i* aus.
